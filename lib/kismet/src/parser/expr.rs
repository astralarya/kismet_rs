...
use super::{
    or_test, stmt_enclosure, target, target_match, token_tag, token_tag_id, ConvertKind, Error,
    ErrorKind, Input, KResult, Token,
};
...
pub fn if_expr(i: Input) -> KResult<Node<Expr>> {
    ...
    let (i, t_block) = stmt_enclosure(i)?;
    let (i, f_block) = opt(preceded(token_tag(Token::ELSE), stmt_enclosure))(i)?;
    match f_block {
        Some(f_block) => Ok((
            i,
            Node::new(
...
pub fn match_expr(i: Input) -> KResult<Node<Expr>> {
    ...
    let i = loop {
        ...
        let (i, val) = opt(stmt_enclosure)(i)?;
        let i = match val {
            Some(val) => {
                arms.push(Node::new(
                    tar.span + val.span,
